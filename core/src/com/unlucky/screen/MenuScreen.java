...
public class MenuScreen extends MenuExtensionScreen {
	...
	public MenuScreen(final Unlucky game, final ResourceManager rm) {
		...
		playLabel = new Label("Play", menuStyle);
		playLabel.setSize(80, 20);
		playLabel.setFontScale(1.5f);
		playLabel.setTouchable(Touchable.disabled);
		playLabel.setAlignment(Align.center);
		playLabel.setPosition(60, 20);
	...
	private void handleOptionButtons() {
		...
		optionLabels = new Label[NUM_BUTTONS];
		String[] labels = new String[] { "Inventory", "Settings", "Shop", "Moves", "Stats", "Credits" };
		for (int i = 0; i < NUM_BUTTONS; i++) {
			...
			optionLabels[i] = new Label(labels[i], menuStyle);
			optionLabels[i].setFontScale(1f);
			optionLabels[i].setTouchable(Touchable.disabled);
			optionLabels[i].setAlignment(Align.center);
			stage.addActor(optionLabels[i]);
		}
		// inventory button
		optionButtons[0].setPosition(6, 85);
		optionLabels[0].setBounds(0, 74, 32, 10);
		// settings button
		optionButtons[1].setPosition(171, 85);
		optionLabels[1].setBounds(165, 74, 32, 10);
		// shop button
		optionButtons[2].setPosition(6, 50);
		optionLabels[2].setBounds(0, 39, 32, 10);
		// smove button
		optionButtons[3].setPosition(6, 15);
		optionLabels[3].setBounds(0, 4, 32, 10);
		// statistics button
		optionButtons[4].setPosition(170, 50);
		optionLabels[4].setBounds(164, 39, 32, 10);
		// credits button
		optionButtons[5].setPosition(170, 15);
		optionLabels[5].setBounds(164, 4, 32, 10);
	...
	private void createCreditsScreen() {
		...
		copyright = new Label("K Pop monster hunters V" + Unlucky.VERSION,
